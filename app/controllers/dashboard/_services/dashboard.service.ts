...
export class DashboardService {
    static async getSummary() {
        ...
        const opnames = await Opname.query().expand(['products']).whereNull('deleted_at').where('status', 'DONE').where('start_at', '>=', startDate).get()
        ...
        const recentOpnamesData = await Opname.query().expand(['products', "brands"]).whereNull('deleted_at').where('status', 'DONE').orderBy('created_at', 'desc').limit(5).get()
